...
class Me(commands.Cog, name="me", command_attrs=dict(hidden=False)):
	...
	async def next_cake(self, ctx, *args:str):
		...
		if len(args):
			try:
				...
				# Insert
				values = ( ("user_id", ctx.author.id), ("user_me", text) )
				await self.database.insert(table_me, values)
				# Update
				await self.database.update(table_me, "user_me", text, "user_id = "+str(ctx.author.id))
	...
	async def show_me_message(self, ctx, user:discord.Member):
		response = await self.database.lookup(table_me, "user_me", "user_id", str(user.id))
