	...
	async def get_command_error(self, ctx, error):
		...
		try:
			...
			if isinstance(error, commands.errors.CommandNotFound):
				await message.edit(f"🕳️ Command `{error.split(' ')[1]}` not found !")
				...
				await message.edit(f"🕳️ Command is on cooldown, wait `{error.split(' ')[7]}` !")
				...
				await message.edit(f"🕳️ Member `{error.split(' ')[1]}` not found ! Don't hesitate to ping the requested member.")
			...
			print(f"! Cog errors get_command_error : {type(error).__name__} : {error}")
